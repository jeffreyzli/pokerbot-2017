...
def getaction(streetDict, potsize, numBoardCards, boardCards, numLastActions, lastActions, numMoves, moves, myHand, button):
    ...
    if aggressive:
        ...
        if betBool and streetDict['5'] < 2:                                         #TODO - Ideally, should prevent more than 2-bet
            betSize = min(potsize * 0.67, (minBet + maxBet) / 2)
            return "BET:" + str(betSize)
        elif raiseBool and streetDict['5'] < 2:                                     #TODO - Ideally, should prevent more than 2-bet
            betSize = min(potsize * 0.67, (minRaise + maxRaise) / 2)
            return "RAISE:" + str(betSize)
        elif odds >= 0.573 and raiseBool and streetDict['5'] < 3:       #PFR to be implemented here. When Aggressive, we want to RAISE 70% of the time and CALL 30% of the time.
